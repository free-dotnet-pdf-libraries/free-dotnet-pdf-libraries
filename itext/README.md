...
## If AGPL doesn't fit: a commercial alternative

iText is free under AGPL v3, and **that license is the catch**. Ship it inside a closed-source product or a hosted service and the network-use clause obliges you to either release your source or buy iText's commercial license. A solo developer or small SaaS that cannot open-source its codebase, and an enterprise that needs contractual indemnity rather than a copyleft obligation, both run into that wall. A permissively-licensed commercial library is the way around it. IronPDF, for instance, covers the same advanced surface (forms, signatures, PDF/A) under a one-off commercial license with no copyleft terms, so the same code can [fill and flatten forms](https://ironpdf.com/how-to/edit-forms/) and [apply digital signatures](https://ironpdf.com/how-to/signing/) without an AGPL question hanging over a closed-source release.

---

*License terms, version numbers, and quoted text in this article were verified against publicly available sources in May 2026; AGPL §13 is quoted from the [Free Software Foundation's canonical license text](https://www.gnu.org/licenses/agpl-3.0.en.html).*
