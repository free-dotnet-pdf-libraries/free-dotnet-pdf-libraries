...
Before the dependency chain, the library deserves its due, because it earned the adoption it got. There are real reasons this was the default choice in 2017–2020 .NET Core projects, and several of those reasons still hold up today on their own terms.
...
So the product is real. The API is good. The friction is low. None of what follows is intended to argue otherwise. The question worth walking through is what is on the other side of the P/Invoke boundary, because that is the half of the picture the install-day experience never shows you.
...
A common framing in 2026 production-readiness discussions is "we're not using wkhtmltopdf, we're using DinkToPdf, which is a .NET library." That framing makes the question feel like a .NET ecosystem question rather than an upstream-archive question. It isn't.
...
There is a gap between "free to install" and "free to operate," and DinkToPdf sits squarely inside it. The DinkToPdf NuGet package is MIT-licensed, costs nothing, and clears procurement review without a conversation. What the procurement review does not surface is the cost of operating a dependency whose .NET wrapper has not shipped a release since April 2017 and whose rendering engine has been archived by its maintainer since 2023.
...
It is worth being specific about where DinkToPdf is still a reasonable choice in 2026, because the dependency-chain story does not turn it into a bad library. It turns it into a library with a clearly bounded fitness window.
...
The line is whether your project sits on the side of that boundary where the upstream-archive question matters, rather than a blanket "use this" or "don't."

## So What Do You Do With This?

Here is the question worth taking away. Open your `.csproj` and look at your DinkToPdf reference. Ask the question the documentation didn't ask for you: do you know what's downstream of that line? Do you know what version of the native binary is in your output directory? Do you know when that binary was last built? Do you know whether your security team, the next time they enumerate your container image's dependencies, is going to ask why an archived project is doing the rendering work in your service?

If the answer to all of those is yes and the trade-off is acceptable, that is a real decision. The aim here is to replace a vibe with a graph you can check yourself, dates and links included, rather than to talk anyone out of DinkToPdf.
...
DinkToPdf's cost shows up at deployment. **It P/Invokes a native wkhtmltopdf binary that is archived upstream**, and the "works on Windows, breaks on Alpine" native-loading failures are a recurring tax on teams shipping to Linux containers. A team that wants HTML-to-PDF to deploy reliably in Docker without hand-managing native libraries, and with a vendor to call when something breaks, benefits from a managed, supported library. A commercial toolkit could cover that need, IronPDF among them, running as a managed .NET package that [auto-configures its Linux and Docker dependencies](https://ironpdf.com/how-to/docker-linux/) and ships [a documented Docker setup](https://ironpdf.com/get-started/ironpdf-docker/), so deployment is a NuGet reference rather than a P/Invoke binary placed per platform.
